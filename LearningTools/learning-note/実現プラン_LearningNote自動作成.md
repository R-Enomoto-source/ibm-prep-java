...
| 対象 | 内容 |
|------|------|
| **`LearningTools/learning-note/create-learning-note.ps1`** | 今日の日付（yyyyMMdd）で `learningNote/LearningNote_yyyyMMdd.md` のパスを組み立て、存在しなければ新規作成。リポジトリルートは `.git` を探して自動検出。 |
| **`.vscode/tasks.json`** | `runOn: "folderOpen"` のタスクを定義し、上記 PowerShell を実行。`presentation.reveal`: `silent`。 |

### 2. スクリプトの仕様（create-learning-note.ps1）

- **配置**: `LearningTools/learning-note/`
- **リポジトリルート**: スクリプト位置から上方向に `.git` があるディレクトリを検出。
- **保存先**: リポジトリルートの `learningNote/LearningNote_yyyyMMdd.md`。
- **ロジック**: 当日ファイルがなければテンプレートから新規作成。既にあれば何もしない。
...
- **保存先**: `learningNote/LearningNote_yyyyMMdd.md`（リポジトリルート直下）。
- **トリガー**: Cursor でこのフォルダを開いたとき（自動タスクを許可したうえで）。
- **重複防止**: スクリプト内で「その日のファイルがなければ作成」のみ行う。

---

## 実装済み（LearningTools 構成）

- **`LearningTools/learning-note/create-learning-note.ps1`** … 当日の `learningNote/LearningNote_yyyyMMdd.md` がなければ新規作成
- **`LearningTools/learning-note/learning-note-template.md`** … 新規作成時のテンプレート
- **`.vscode/tasks.json`** … フォルダーを開いたときに上記スクリプトを実行（パスは `LearningTools/learning-note/create-learning-note.ps1` を参照）
