...
1. **生成AI が「指定」で示した画像フォルダ内の各画像を順に開き、画像を直接読み取って（OCR）正確なテキストを取得する**。スクリプトやコマンドは使わず、画像を見て内容を書き起こす。取得したテキストの内容を判別する。
...
- 第1・2段階の結果: 「指定」で示した問題文用・解答解説用のファイルにテキストが保存されていること。
