...
2. `[ ]` で囲んだ部分を、実際の **問題文用パス**・**解答解説用パス**・**画像フォルダの相対パス** に置き換える。
3. AI（または作業者）に渡して実行する。
...
## 入力
- **画像の場所**: [画像フォルダの相対パスまたは絶対パス]
  - 例: `local_data\pdf_output\3章__JavaSE17Silver_黒本_〜_コピー_images`
...
5. **照合のやり方**
   - 保存した問題文・解答解説の **各セクション（各問）** について、対応する元画像を再度開く。
   - 次の項目を **1問ずつ** 確認する:
     - 問題番号・見出しの一致
...
| プレースホルダ | 説明 | 例 |
|----------------|------|-----|
| `[画像フォルダの相対パスまたは絶対パス]` | ページ画像が入ったフォルダ | `local_data\pdf_output\3章__JavaSE17Silver_黒本徹底攻略…_コピー_images` |
...
すでにテキスト化済みで、「照合チェックだけ」行いたいとき用の短いプロンプトです。
...
手順:
1. 各問について、対応する画像を開き、問題文・コード・選択肢・正解を比較する。
