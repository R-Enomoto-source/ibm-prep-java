...
この「どちらか一方を選ぶ」というところまでは分かるが、**「結局いつも“小さい方”を選んでいる」**には、なかなか気づきにくいかもしれません。

そこで、上の2パターンを**「どっちの数が使われているか」**で見直してみます。

| 場合 | 条件（大きさの関係） | 実際に使う量 |
|------|----------------------|--------------|
| 超えないとき | やりたい量 **≦** やってもいい量（やりたい量のほうが小さい or 同じ） | **やりたい量** |
| 超えるとき   | やりたい量 **＞** やってもいい量（やってもいい量のほうが小さい）       | **やってもいい量** |

どちらの場合も、**使っているのは「2つの中で小さいほう」**になっています。  
「超えないときは recover、超えるときは MAX_MP - mp」と別々に覚えるよりも、**「いつでも recover と (MAX_MP - mp) の小さい方を使う」**とまとめて考えると、後のコード（min）にもつながりやすいです。

ということで、**いつでも「やりたい量」と「やってもいい量」のどちらか小さい方を使う**、と一言で書けます。

ここまで分かったあと、「じゃあそれをコードでどう書く？」で詰まりがちな部分です。

- **「2つの値のうち、小さい方を使う」** ＝ プログラミングでは **`min(値A, 値B)`** という「2つの引数のうち小さい方を返す処理」で表せます。
- 今回なら「小さい方を使う」の**値A**＝やりたい量（`recover`）、**値B**＝やってもいい量（`MAX_MP - mp`）なので、

  **実回復量 = min(recover, MAX_MP - mp)**

と書けば、超えないときは `recover`、超えるときは `MAX_MP - mp` が自動で選ばれます。「if で分岐して代入」を書かずに、**「小さい方を取りたい」＝ min に渡す**と覚えておくと、同じ形の処理をすぐコードに落としやすくなります。
