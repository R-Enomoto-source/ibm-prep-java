...
# ログ関数（日別ファイル: log-yyyy-MM-dd.txt）
function Write-Log {
    ...
    try {
        ...
        # UTF-8 BOM付きで書き込み（既存ファイルがない場合のみBOMを付ける）
        $utf8WithBom = New-Object System.Text.UTF8Encoding $true
        if (-not (Test-Path $logFile)) {
            # 新規ファイルの場合はBOM付きで書き込み
            [System.IO.File]::AppendAllText($logFile, $logMessage + "`r`n", $utf8WithBom)
        } else {
            # 既存ファイルの場合はBOMなしで追記
